...
The `syncBaseFields` worker is designed to synchronize basefields from a remote to a local instance of the pdc service. An administrative user looking to seed their database can make a POST request to the `/syncBaseFields` route, specifying the remote url in the body of the form:

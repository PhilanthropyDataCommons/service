...
integrated system, for example `okta-oidc-foundation` or `okta-saml-foundation`,
...
1. Log in to the admin area to Start an OIDC App Integration (these steps follow
...
2. On the left nav bar, visit "Directory" -> "Groups"
3. Click "Add group"
4. Set "Name" to "PDC"
5. Set "Description" to "Users who may access the Philanthropy Data Commons."
6. Click "Save"
7. Click the newly created "PDC" group
8. Click "Assign people" and add at least one user.
...
6. Click "Next."
7. Add "App Name" of "Philanthropy Data Commons."
8. Leave "Proof of possession" unchecked.
9. Leave "Login initiated by App Only" selected.
10. Set "Grant type" to "Authorization code" only.
11. Set "Sign-in redirect URI" to the endpoint provided by the PDC team, similar
    to "https://example.org/realms/pdc/broker/okta-oidc-foundation/endpoint"
    **Important**: the name following `broker/` here needs to match the agreed
    alias mentioned above.
12. Clear the "Sign-out redirect URIs" by clicking the X.
13. Under "Assignments" select "Limit access to selected groups"
14. Set the "Selected group(s)" to the "PDC" group created earlier.
15. Click "Save"
16. Share the "Client ID" from "Client Credentials" with the PDC team.
17. Under "Client Credentials" click "Edit"
18. Set "Client authentication" to "Public key / Private key"
19. Check "Require PKCE as additional verification"
20. Under "Public Keys" -> "Configuration", set "Use a URL to fetch keys..."
21. Set the URL to the JWKS URL of the PDC Keycloak server, provided by the PDC
    team, similar to
    "https://example.org/realms/pdc/protocol/openid-connect/certs"
22. Click "Save"
23. Click "Save" at the "Existing client secrets will no longer be used" warning

#### Create an Access Policy and Rule for the Authorization Server

9. On the left nav bar, visit "Security" -> "API" (way down there)
10. Click the desired Authorization Server name, usually "default"
11. Copy the "Metadata URI" value and send it to the PDC team.
12. Click the "Access Policies" tab
13. Click "Add New Access Policy"
14. Set "Name" to "PDC"
15. Set "Description" to "Allow users to authenticate to PDC"
16. Set "Assign to" to "The following clients:"
17. Type "P" in the input box and then pick the PDC client created above.
18. Click "Create Policy"
19. Under the new PDC policy, click "Add rule"
20. Set the "Rule Name" to "Issue tokens to users authenticating to PDC"
21. Reduce the token lifetimes such as to 5 minutes (access) and 90 minutes.
22. Click "Create rule"
...
8. Check "Do not display application icon to users" under "App visibility" to
   avoid erroneous IdP-initiated login attempts. Users begin the login flow
   from PDC Apps, not from the Okta IdP.
9. Click "Next."
...
3. Set the "Alias" to the descriptive, simple, unique alias named above, e.g.
   "okta-oidc-foundation", **Important**: this alias sets the broker or
...
31. Keep "Hide on login page" set to "On."

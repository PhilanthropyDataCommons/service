...
describe('/users/changemakers/:changemakerId/permissions/:permission', () => {
	...
	describe('DELETE /', () => {
		...
		it('returns 204 if the permission had previously been deleted', async () => {
			const user = await loadTestUser();
			const changemaker = await createChangemaker({
				taxId: '11-1111111',
				name: 'Example Inc.',
		...
		it('deletes the user changemaker permission when the user has administrative credentials', async () => {
			...
			await request(app)
				...
				.send()
				.expect(204);
			const deletedPermission = await loadUserChangemakerPermission(
				user.keycloakUserId,
				changemaker.id,
				Permission.EDIT,
			);
			expect(deletedPermission).toEqual({
				changemakerId: changemaker.id,
				createdAt: expectTimestamp,
				createdBy: user.keycloakUserId,

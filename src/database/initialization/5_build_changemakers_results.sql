...
CREATE FUNCTION build_changemakers_results(
	...
	-- gold field value per base field, per changemaker
	gold_field_json AS (
		SELECT
			gold.changemaker_id,
			jsonb_agg(gold.field_value_json ORDER BY gold.base_field_short_code)
				AS fields
		FROM (
			SELECT DISTINCT ON (combined.changemaker_id, combined.base_field_short_code)
				combined.changemaker_id,
				combined.field_value_json,

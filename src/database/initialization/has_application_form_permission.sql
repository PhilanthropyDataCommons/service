CREATE OR REPLACE FUNCTION has_application_form_permission(
...
BEGIN
	...
	-- Check if the user has the specified permission on the specified application form
	-- via direct user grant or group membership
	SELECT EXISTS (
		SELECT 1
		FROM permission_grants pg
		WHERE pg.context_entity_type = 'applicationForm'
			AND pg.application_form_id
				= has_application_form_permission.application_form_id
			AND has_application_form_permission.permission = ANY(pg.verbs)
			AND has_application_form_permission.scope = ANY(pg.scope)
			AND (
				(

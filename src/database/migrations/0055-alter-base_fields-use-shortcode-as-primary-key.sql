...
-- 4. Drop the existing foreign key constraint on base_field_id.
ALTER TABLE base_field_localizations
DROP CONSTRAINT base_field_localizations_base_field_id_fkey;
ALTER TABLE application_form_fields
DROP CONSTRAINT fk_base_field;

-- 5. Drop the existing primary key constraint on base_field_localizations.
ALTER TABLE base_field_localizations
DROP CONSTRAINT base_field_localizations_pkey;

-- 6. Drop the old foreign key column base_field_id.
ALTER TABLE base_field_localizations
DROP COLUMN base_field_id;
ALTER TABLE application_form_fields
DROP COLUMN base_field_id;

-- 7. Add the new primary key on (base_field_short_code, language)
ALTER TABLE base_field_localizations
ADD PRIMARY KEY (base_field_short_code, language);

-- 8. On the base_fields table, drop the old primary key constraint.
ALTER TABLE base_fields
DROP CONSTRAINT canonical_fields_pkey;

-- 9. Add a new primary key on the "short_code" column.
ALTER TABLE base_fields
ADD PRIMARY KEY (short_code);

-- 10. Drop the old "id" column from base_fields.
ALTER TABLE base_fields
DROP COLUMN id;

-- 11. Add a new foreign key constraint.
ALTER TABLE base_field_localizations

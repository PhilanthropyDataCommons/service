...
describe('mergeProposalFieldValues', () => {
	it('should merge proposal values into proposal version', () => {
		...
		const proposalVersionWithFieldValues =
			joinProposalFieldValuesToProposalVersion(proposalVersion, fieldValues);
	...
	it('should return a proposal version like the one given when no field values are given', () => {
		...
		expect(
			joinProposalFieldValuesToProposalVersion(proposalVersion, []),
		).toEqual({
			id: 3413,
			proposalId: 3433,
			applicationFormId: 3449,

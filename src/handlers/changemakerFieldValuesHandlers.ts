...
const postChangemakerFieldValue = async (
	...
	const { changemakerId, baseFieldShortCode, sourceId, value, goodAsOf } = body;
	...
	if (baseField.category !== BaseFieldCategory.ORGANIZATION) {
		throw new UnprocessableEntityError(
			`Base field ${baseField.shortCode} has category '${baseField.category}' but must have category 'organization' for changemaker field values.`,
		);
	}

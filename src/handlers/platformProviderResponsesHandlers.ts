...
const getPlatformProviderResponsesByExternalIdParamsSchema
: JSONSchemaType<GetPlatformProviderResponseByExternalIdParams> = {
  type: 'object',
  properties: {
    externalId: {
      type: 'string',
...
const getPlatformProviderResponsesByExternalId = (
  ...
  db.sql<PlatformProviderResponse>(
    ...
    const { rows: platformProviderResponses } = platformProviderResponsesQueryResult;
    res.status(200)
...
const isPostPlatformProviderResponseSchema = ajv.compile(postPlatformProviderResponseSchema);
const postPlatformProviderResponse = (
  ...
  db.sql<PlatformProviderResponse>('platformProviderResponses.insertOne', req.body)

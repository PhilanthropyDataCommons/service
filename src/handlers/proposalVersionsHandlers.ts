...
const assertApplicationFormExistsForProposal = async (
  ...
  if (applicationForm === undefined) {
    throw new InputConflictError(
      'The Application Form does not exist.',
      {
  ...
  const proposalsQueryResult = await db.sql<Proposal>(
...
const assertProposalFieldValuesMapToApplicationForm = async (
  ...
  const applicationFormFieldQueries = proposalFieldValues.map(
    async (proposalFieldValue) => db.sql<ApplicationFormField>(
...
const postProposalVersion = (
  ...
  Promise.all([
    ...
    db.transaction(async (transactionDb) => {
      const proposalVersionQueryResult = await transactionDb.sql<ProposalVersion>('proposalVersions.insertOne', req.body);
      ...
      if (isProposalVersion(proposalVersion)) {
        const proposalFieldValueQueries = req.body.fieldValues.map(
          async (fieldValue) => transactionDb.sql<ProposalFieldValue>(
        ...
        const proposalFieldValueResults = await Promise.all(proposalFieldValueQueries);
      ...
      if (isTinyPgErrorWithQueryContext(error)) {
        next(new DatabaseError(
          'Error creating proposal version.',
          error,
        ));
        return;
      }

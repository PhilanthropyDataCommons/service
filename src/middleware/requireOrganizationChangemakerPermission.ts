...
import { db, loadOrganizationChangemakerPermission } from '../database';
import type { Permission } from '../types';
import type { Request } from 'express';

const requireOrganizationChangemakerPermission =
	...
	(req: Request, res: Response, next: NextFunction) => {
		...
		const { changemakerId, keycloakOrganizationId } = req.params;
		...
		const { user } = req;
		const userpermissions = user.permissions.changemaker[changemakerId] ?? [];

		if (!userpermissions.includes(permission)) {
			loadOrganizationChangemakerPermission(
				db,
				null,
...
export { requireOrganizationChangemakerPermission };

...
import { db, loadOrganizationFunderPermission } from '../database';
import type { Permission } from '../types';
import type { Request } from 'express';

const requireOrganizationFunderPermission =
	...
	(req: Request, res: Response, next: NextFunction) => {
		...
		const { funderShortCode, keycloakOrganizationId } = req.params;
		...
		const { user } = req;
		const userpermissions = user.permissions.funder[funderShortCode] ?? [];

		if (!userpermissions.includes(permission)) {
			loadOrganizationFunderPermission(
				db,
				null,
...
export { requireOrganizationFunderPermission };

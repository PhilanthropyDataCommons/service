...
const getS3Endpoint = async (bucketName: string) => {
...
const mockS3GetObjectReplyWithFile = async (file: File, filePath: string) =>
	nock(await getS3Endpoint(file.bucketName))
		.get(getS3KeyPath(file.bucketName, file.storageKey))
...
describe('processBulkUploadTask', () => {
	...
	it('should fail if the proposalsDataFile is not accessible', async () => {
		...
		const sourceRequest = nock(await getS3Endpoint(proposalsDataFile.bucketName))
			.get(
				getS3KeyPath(proposalsDataFile.bucketName, proposalsDataFile.storageKey),
			)
			.query({ 'x-id': 'GetObject' })
			.reply(404);

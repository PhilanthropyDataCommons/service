...
interface ControlColumnHeader {
	csvIndex: number;
	key: string;
}

interface ClassifiedColumnHeaders {
	dataColumnHeaders: DataColumnHeader[];
	controlColumnHeaders: ControlColumnHeader[];
}
...
export const classifyHeaderRow = (
	headerRow: string[],
): ClassifiedColumnHeaders => {
	const dataColumnHeaders: DataColumnHeader[] = [];
	const controlColumnHeaders: ControlColumnHeader[] = [];
	headerRow.forEach((label, csvIndex) => {
		if (label.startsWith(CONTROL_COLUMN_PREFIX)) {
			...
			controlColumnHeaders.push({ csvIndex, key });
		} else {
			dataColumnHeaders.push({ csvIndex, label });
		}
	});
	return { dataColumnHeaders, controlColumnHeaders };
};

export const assertControlColumnsAreValid = (
	classifiedColumnHeaders: ClassifiedColumnHeaders,
): void => {
	const seenKeys = new Set<string>();
	classifiedColumnHeaders.controlColumnHeaders.forEach(({ key }) => {
...
export const extractControlValues = (
	...
	controlColumnHeaders.forEach(({ csvIndex, key }) => {
		const { [csvIndex]: value } = record;
		if (value === undefined || value === '') {
			return;
		}
		if (value !== value.trim()) {
			throw new InputValidationError(
				`Row ${recordNumber}: control column "${CONTROL_COLUMN_PREFIX}${key}" ` +
					`value "${value}" has leading or trailing whitespace. ` +
					`Remove the whitespace or leave the cell empty.`,
				[],
			);
		}
...
const validateAndClassifyColumns = async (
	csvPath: string,
	applicationForm: ApplicationForm,
): Promise<ClassifiedColumnHeaders> => {
	const headerRow = await loadHeaderRowFromCsv(csvPath);
	const classifiedColumnHeaders = classifyHeaderRow(headerRow);
	assertControlColumnsAreValid(classifiedColumnHeaders);

	const { dataColumnHeaders } = classifiedColumnHeaders;
	const expectedLabels = applicationForm.fields.map((field) => field.label);

	if (dataColumnHeaders.length !== expectedLabels.length) {
		throw new InputValidationError(
			`CSV has ${dataColumnHeaders.length} data columns but the application form has ${expectedLabels.length} fields.`,
			[],
		);
	}

	dataColumnHeaders.forEach(({ label, csvIndex }, index) => {
	...
	return classifiedColumnHeaders;
};
...
const getDataColumnCsvIndexForShortCode = (
	...
	return dataColumnHeaders[fieldIndex]?.csvIndex;
};
...
export const processBulkUploadTask = async (
	...
	try {
		...
		const { dataColumnHeaders, controlColumnHeaders } =
			await validateAndClassifyColumns(
		...
		await db.transaction(async (transactionDb) => {
			...
			await parser.forEach(async (record: string[]) => {
				...
				await allNoLeaks(
					dataColumnHeaders.map<Promise<ProposalFieldValue>>(
